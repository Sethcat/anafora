...
PropertyFrame.updateSpanValue = function(e) {
	...
	console.log('updateSpanValue: spanListIdx:', spanListIdx, 'spanIdx:', spanIdx, 'oldSpan:', oldSpan);

	if(spanIdx == 1) {
		console.log('span left');
...
PropertyFrame.prototype.generatePropertyTableRow = function(pType, pIdx, pValue, isDiffProp, comparedIdx) {
	...
	switch(pType.input) {
		case InputType.LIST:
			//comment below so that a user can't delete entity from propertyFrame
			...
			//else 
			{
				$.each(pValue, function(listIdx, listElement) {
					...
					//modify: don't show DeleteBtn
					tValue += '<div class="propertySubAObj">' + listElement.genElementStr() + "</div>";
	...
	/*tRow.find(".propertySubAObj").bind({
		mouseenter: function(evt) {
			...
			if(currentAProject.selectedAObj instanceof AdjudicationEntity || currentAProject.selectedAObj instanceof AdjudicationRelation) {
				...
				console.log('pIdx', pIdx, 'pListIdx', pListIdx);
				hoverAObj = currentAProject.selectedAObj.propertyList[pIdx][pListIdx]
			//comment below so that a user can't delete entity from a propertyFrame
...
PropertyFrame.propertySelectMenu = $.contextMenu({
		...
		events: {
			hide: function(opt) {
				...
				//console.log('property Value choice:'+ $($trigger).text());
				//$("#pValue").text($($trigger).text());
			}
		}
	});
