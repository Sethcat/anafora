...
function onLoad() {
	...
	fileMenu.eq(0).bind("click", function() { 
		if(getIsChanged() && window.confirm("Save Task?")){var str= is_fill_property_value()
			if(str=='yes'){ saveFile();}
			else{alert(str)}} projectSelector.selectProject(); projectSelector.popup(); });
	fileMenu.eq(1).bind("click", function() { 
		var str= is_fill_property_value()
		if(str=='yes'){ saveFile();}
		else{alert(str)}
		 });
    ...
    fileMenu.eq(3).bind("click", function() {
		var str= is_fill_property_value()
		if(_setting.isAdjudication){ currentAProject.adjudicationCompleted();} 
		else if(str=='yes'){ temporalSave();saveFile();setCompleted();projectSelector = new ProjectSelector(_setting);projectSelector.popup();}
		else{alert(str)}});	
...
function is_fill_property_value(){
	var type_list= $($.parseXML(currentAProject.writeXML())).find("entity")
	var evaluation_list= $($.parseXML(currentAProject.writeXML())).find("评价")
	var type_array = new Array();
	var evaluation_array = new Array();
	var undone_array= new Array();
	var back_array=new Array();
	var back_str="yes"
	var type_str=""
	type_list.each(function(){
		type_array[type_array.length]=$(this).find("type").text()
	   })

	evaluation_list.each(function(){
		evaluation_array[evaluation_array.length]=$(this).text()
	   })
/* 	for (var i = 0; i < evaluation_array.length; i++ ){
		if(evaluation_array[i] == ''){
			undone_array[undone_array.length]=type_array[i]
		}
	} */
	for (var i = 0; i < evaluation_array.length; i++ ){
		if(evaluation_array[i] == ''){
			if(type_str==""){
				type_str="@【"+$.trim(type_array[i])+"】"
			}
			else{
				if(type_str.indexOf($.trim(type_array[i]))==-1){
					type_str=type_str+"@【"+$.trim(type_array[i])+"】"
				}
									
			}		
			}
	if(!type_str ==""){
		back_str="请注意！以下类型属性值没有填写，请仔细查看，并将属性值填上，否则已标注结果将不能保存在服务器上。具体未标注类型如下："+type_str
	}
	}
...
function loadNewProject() {
	console.log('function loadNewProject');
	// set editable
	...
	annotatorDiv.bind("mouseup", function(evt) { 
		if ((window.getSelection && window.getSelection().toString()!=="" ) || (document.selection && document.selection.createRange().text !== "" ) ){
	...
	displayRelationList = [];
	//displayRelationList 改成全局变量
	//_setting.displayRelationList = [];
	...
	if(displayRelationList != undefined && displayRelationList.length > 0) {
		//relationFrame.displayRelations(displayRelationList.slice(0,1));
		showRelationFrame();
	}
	...
	// set timer
	if(editable)
		setInterval( saveFileByTime, 10000);
...
function saveFileByTime() {
	var str= is_fill_property_value();
	if(getIsChanged() && editable && str=='yes') {
		$.ajax({type: 'POST', url:  _setting.root_url + "/" + _setting.app_name + "/saveFile/" + _setting.projectName + "/" + _setting.corpusName + "/" + _setting.taskName + "/" + _setting.schema + (_setting.isAdjudication ? ".Adjudication" : "") + "/", data: {'fileContent':localStorage["anafora"]}, cache: false, async: false, headers:{"X-CSRFToken":$.cookie('csrftoken') }, success: function(data) {setIsChanged(false);localStorage.removeItem("anafora");},error: function (xhr, ajaxOptions, thrownError) { console.log(xhr.responseText); }} );
		console.log('save file by every 10s!');
	}else if(str != 'yes'){alert(str)}
}
...
function annotateClick(evt) {
	...
	console.log('----annotateClick----');
	console.log(overlapIdx);
	overlapIdx = _setting.startrecord[_setting.startrecord.length-overlapIdx-1];
	console.log('1 mouse click:',overlapIdx);
	var aObjList = currentAProject.annotateFrame.overlap[overlapIdx].aObjList;
	
	console.log('2 aObjList', aObjList);
	...
	console.log('3 matchChecked:', matchChecked);
...
function showAObjProperty(aObj) {
	...
	console.log('propertyFrameList', propertyFrameList);
